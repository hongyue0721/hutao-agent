...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|hutao] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Update hutao and installed packages.

Options:
  --self                  Update hutao only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall hutao even if the current version is latest

Short forms:
  ${APP_NAME} update                Update hutao and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update hutao          Update hutao only (self works as alias to hutao)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "hutao" || source === "pi";
			if (sourceIsSelf) {
